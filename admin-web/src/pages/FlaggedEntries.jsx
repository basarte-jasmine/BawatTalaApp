    ...
    <Layout title="Flagged Entries" subtitle="Review journal entries that need counselor attention and follow-up." onLogout={onLogout} session={session}>
      <div className="mx-auto max-w-[1200px] space-y-6 pb-12">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            },
            {
              ...
              bgClass: "bg-red-600",
              cardClass: "border-red-300 bg-red-600 shadow-lg shadow-red-100",
              valueClass: "text-white",
              labelClass: "text-red-50",
            },
            {
            ...
            return (
              <div key={stat.label} className={`flex items-center gap-4 rounded-2xl border p-5 shadow-sm ${stat.cardClass}`}>
                <div className={`flex h-12 w-12 items-center justify-center rounded-full ${stat.bgClass}`}>
                  <Icon className={`h-6 w-6 ${stat.iconClass}`} />
                </div>
                <div>
                  <div className={`text-sm font-medium ${stat.labelClass || "text-slate-600"}`}>{stat.label}</div>
        ...
        <div className="overflow-hidden rounded-2xl border border-red-200 bg-white shadow-sm">
          <div className="flex flex-col items-center justify-between gap-4 border-b border-red-200 bg-red-50 p-4 sm:flex-row">
            <div className="flex items-center space-x-1 rounded-xl bg-red-100 p-1">
              {TABS.map((tab) => (
                <button
                  key={tab}
                  type="button"
                  ...
                  className={`rounded-lg px-4 py-1.5 text-sm font-medium ${
                    activeTab === tab
                      ...
                      : "text-red-800 hover:bg-white hover:text-red-900"
                  }`}
                >
                  {tab}
                </button>
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                ...
                className="w-full rounded-xl border border-red-200 bg-white py-2 pl-9 pr-4 text-sm focus:border-transparent focus:outline-none focus:ring-2 focus:ring-red-500"
              />
            </div>
          </div>
                      ...
                      >
                        <td className="whitespace-nowrap px-6 py-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <div className="font-semibold text-slate-950 group-hover:text-red-700">{entry.fullName}</div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {Array.isArray(entry.concernTags) && entry.concernTags.length
                              ? entry.concernTags.slice(0, 2).map((tag) => (
                                  <span key={`${entry.id}-${tag}`} className="inline-flex rounded-full border border-orange-200 bg-orange-50 px-2.5 py-1 text-xs font-medium text-orange-800">
                                    {tag}
                                  </span>
                                ))
                              : null}
          ...
          {selectedEntry ? (
            <div className="space-y-6">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                  <h3 className="text-xl font-bold text-red-950">{selectedEntry.fullName}</h3>
              ...
              <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1.35fr,1fr]">
                <div className="space-y-4">
                  <div className="rounded-2xl border border-red-200 bg-red-50 p-4">
                    <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-red-700">Summary</div>
                ...
                <div className="space-y-4">
                  <div className="rounded-2xl border border-slate-200 bg-white p-4">
                    <div className="mb-2 text-xs font-semibold uppercase tracking-wider text-red-700">Concern Tags</div>
                    <div className="flex flex-wrap gap-2">
                      {Array.isArray(selectedEntry.concernTags) && selectedEntry.concernTags.length ? (
                        selectedEntry.concernTags.map((tag) => (
                          <span key={`${selectedEntry.id}-${tag}`} className="rounded-full bg-red-50 px-3 py-1 text-xs font-medium text-red-700">
                            {tag}
                          </span>
                        ))
                      ) : (
        ...
        <Modal isOpen={Boolean(messageTarget)} onClose={() => setMessageTarget(null)} title={`Message ${messageTarget?.fullName || ""}`}>
          <div className="space-y-4">
            <div className="flex items-start gap-3 rounded-xl border border-red-100 bg-red-50 p-3">
              ...
              <p className="text-sm text-red-800">This message will be sent to the student's mobile notification center.</p>
            </div>
